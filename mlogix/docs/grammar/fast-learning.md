...
### 💡科学计数法浮点数(Num)
用科学计数法表示的浮点数(*即使实际是整数也会当成浮点数*)  
在浮点数后加上`e`或`E`，再加上一个数(可携带正负号)  
...
#### 十六进制表示
表示颜色的值，底层上是Num，且`== 0`  
`0%`开头，字符范围`0~9` `a~f` `A~F`，**长度限制**为6或8  
...
set a = out              # ✅
set b = message1        # ❌找不到`message1`的标识符
set message1             # ⚠️合法，但可能引起误会
```
```mlx
use link.{
    message1 as out # 用于输出结果
...
MLogiX是一门静态类型语言，每个值都有它的类型。 但你基本不用自己写，因为大多数时候编译器帮你自动补充了变量的类型。
...
*`match` 对枚举的**穷尽性检查**（是否覆盖全部变体）已实现，见下一章。*

---

## 12. ⭐模式匹配 (match)

`match` 把一个值和一组**模式**逐个比较，第一个匹配上的分支体被执行，后面的分支不再看。
分支写法是 `模式 -> 语句`（通常是一条代码块）。
...
|:---|:---|:---|
| 通配符 | `_` | 匹配任何值，不绑定 |
| 绑定 | `x`（裸标识符） | 匹配任何值，并把值绑到新变量 `x` |
| 变体 | `枚举名.变体` / `枚举名.变体(p1, p2)` | 匹配该枚举的那个变体；载荷按**声明顺序**用子模式解构，可嵌套 |
...
match o {
    Option.Some(x) -> { print(x) }   # x 绑定载荷，类型就是 T 实例化后的类型
    Option.None -> { print("空") }
}
```
...
**裸标识符一定是绑定**，不会去匹配变体：写 `None` 只是绑定了一个叫 `None` 的新变量
（编译器会告警提醒你可能想写 `Option.None`）。

### 💡绑定与作用域

- 绑定只在**该分支**（模式 + 分支体）内可见，出了分支就没了；
- 不同分支的绑定互不影响，允许与外面的同名变量遮蔽；
- 子模式里的绑定（如 `Option.Some(Option.Some(x))` 的 `x`）同样只属于该分支。

### ⭐穷尽性：枚举必须列全

被匹配值的类型是枚举时，分支必须覆盖**全部变体**（或用 `_`/绑定兜底），否则报错，
并在消息里给出一个没有被覆盖的取值作为例子：

```
error: `match` 未覆盖所有取值: `Option.Some(_)` 未被匹配
note: 被匹配值的类型是 `Option<Int>`，其变体有: None, Some
```

嵌套模式也会被精确检查：只写 `Option.Some(Option.None)` 并不能覆盖
`Option.Some(Option.Some(_))`，编译器会把它算作缺口。

### 💡冗余分支（告警）

前面的分支已经覆盖的取值再写分支就是不可达的，编译器会告警：重复写同一个变体、
或者在 `_`/绑定之后还写别的分支都会触发。

### 🔬还不是模式的部分

字面量（`1`、`1.5`、`"s"`、`true`、`null`）、元组 `(a, b)`、范围 `1 :< 5`、or 模式 `A | B`、
guard（`模式 if 条件 ->`）都还没支持；写出来会得到「期望模式」的报错与一段提示。
